	...
	function Schedule(opt) {
		...
		var bindEvent = function() {
			el.addEventListener('click', function(e) {
				...
				//console.log(e.target.className.indexOf('select_color'));
		...
		var init = function() {
			var scheduleHd = '<div class="schedule-hd">' +
				...
				'</div>' +
				'</div>'
			var scheduleWeek = '<ul class="week-ul ul-box" style="text-align: center;padding: 15px 0;font-size: 2.5rem;">' +
				'<li>日</li>' +
				'<li>一</li>' +
				'<li>二</li>' +
				'<li>三</li>' +
				...
				'<li>六</li>' +
				'</ul>'
			var scheduleBd = '<ul class="schedule-bd ul-box" style="margin-top: 1rem;"></ul>';
			el.innerHTML = scheduleHd + scheduleWeek + scheduleBd;
			bindEvent();
			var _userData = JSON.parse(getLsItem("mopcData"));
			//console.log(_userData);
			var m2 = (month + 1) < 10 ? ("0" + (month + 1)) : (month + 1);
			mAjax(_url + 'learServer/getInspectList', {
				month: year + "-" + m2,
		...
		var render = function(data) {
			...
			for(var i = 0; i < total; i++) {
				if(i < startWeek) {
					...
					var addClass = '';
					var initClass = '';
					var _num = data[i - startWeek].checkNum;
					if(data.length > 0) {
					...
					eleTemp.push('<li class="current-month" ><span title=' + nowDate + ' class="currentDate dayStyle ' + addClass + '">' + (i + 1 - startWeek) + '</span><span class="' + initClass + '"><span class="num">' + _num + '</span></span></li>')
				} else {
					//console.log(88888);
